...
if ( ! class_exists( 'SHPG_Register_Base_Shortcode' ) ) {
	abstract class SHPG_Register_Base_Shortcode implements SHPG_Register {
		use SHPG_Hook_Impl;
		...
		public function register(): void {
			foreach ( $this->get_items() as $item ) {
				if ( $item instanceof SHPG_Reg_Shortcode ) {
		...
		public function heading_actions_handler(): void {
			if ( $this->heading_actions && is_singular() ) {
				...
				foreach ( array_unique( $this->found_tags ) as $tag ) {
					$callback = shpg_parse_callback( $this->heading_actions[ $tag ] );
		...
		public function dispatch( $atts, string $enclosed, string $tag ): string {
			$callback = shpg_parse_callback( $this->real_callbacks[ $tag ] ?? '__return_empty_string' );
