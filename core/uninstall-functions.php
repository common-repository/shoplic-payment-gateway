...
if ( ! function_exists( 'shpg_cleanup_meta' ) ) {
	/**
	 * Remove plugin's meta records.
	 */
	function shpg_cleanup_meta() {
		global $wpdb;
...
if ( ! function_exists( 'shpg_cleanup_option' ) ) {
	/**
	 * Remove plugin's option records.
	 */
	function shpg_cleanup_option() {
		global $wpdb;

		$option_names = [];

		$option = shpg()->registers->option;
		if ( $option ) {
			foreach ( $option->get_items() as $item ) {
				if ( $item instanceof SHPG_Reg_Option ) {
...
if ( ! function_exists( 'shpg_cleanup_terms' ) ) {
	/**
	 * Remove plugin's terms.
	 */
	function shpg_cleanup_terms() {
		global $wpdb;

		$taxonomies = [];

		$taxonomy = shpg()->registers->taxonomy;
		if ( $taxonomy ) {
			foreach ( $taxonomy->get_items() as $item ) {
				if ( $item instanceof SHPG_Reg_Taxonomy ) {
...
if ( ! function_exists( 'shpg_cleanup_posts' ) ) {
	/**
	 * Remove plugin's posts.
	 */
	function shpg_cleanup_posts() {
		global $wpdb;

		$post_types = [];

		$post_type = shpg()->registers->post_type;
		if ( $post_type ) {
